...
from memra.domain.services.intent import Classification
...
async def _resolve_sources(chunks: list[dict], db_session_factory) -> list[dict]:
    ...
    Iterates all chunks in order, looks up each by UUID (LightRAG file_path),
    deduplicates by document id, and assigns sequential ref numbers.
...
async def _persist_messages(
    ...
    """
    from memra.domain.services import conversations as svc
    try:
        async with db_session_factory() as session:
...
async def _retrieve(
    ...
    """
    from memra.domain.services.lightrag_service import CORPUS_ID as _CID, retrieve_chunks

    effective_corpus = corpus_key or _CID
    chunks = await retrieve_chunks(effective_corpus, message, settings, mode=mode, org_id=org_id)
    return [
        {
...
async def stream_response(
    ...
    if corpus_key is None and org_id and db_session_factory:
        try:
